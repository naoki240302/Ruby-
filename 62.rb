...
def show_cars(cars)
  ...
  if car
    show_data(car)
    puts "これから走る予定の距離(km)を整数で入力してください"
    input_distance = gets.to_i
    calc_fuel(input_distance, car)
  else
    puts "該当する番号はありません。"
  end
end
...
def calc_fuel(distance, car)
  fuel = distance / car[:fuel_economy]
  puts "これから走行する距離は#{distance}kmで、必要なガソリンは#{fuel}Lです"
end
